...
const MapInfo = () => {
  ...
  const baseStorageURL = api.defaults.baseURL.replace("/api", "");
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl">
              ...
              mapData.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-left">
                    {item.image ? (
                      <img
                        src={`${baseStorageURL}/storage/mapinfo/${item.image}`}
                        alt="Map"
                  ...
                  {field.type === "file" ? (
                    <>
                      ...
                      {currentItem?.image && !imageRemoved && !imageFile && (
                        <div className="mt-2">
                          <img
                            src={`${baseStorageURL}/storage/mapinfo/${currentItem.image}`}
                            alt="Current"
      ...
      {showToast && (
        <div
          className={`fixed bottom-4 right-4 p-4 text-white rounded shadow ${getToastColor(toastType)}`}
        >
          {toastMessage}
        </div>
      )}
